...
const useSwap = (props) => {
  ...
  console.log("token1_bal", token1_bal);
  ...
  const updateTokenList = (list) => {
    ...
    const addTokenToList = (i) => {
      if (rEqual(i, list.length)) {
        return tokenList.length && dispatch(addTokensToTokenList(tokenList));
      }
      searchOrImportToken(list[i])
        .then(async (token) => {
          await tokenList.push(token);
          console.log("tokenListTTTTT", tokenList);
          await addTokenToList(i + 1);
        })
        .catch((e) => {
          throw e;
        });
  ...
  useEffect(
    (_) => {
      ...
      if (lock.current && !window.isPageForcedToReload && wallet.isConnected) {
        ...
        retrieveTokenList().then((res) => {
          console.log(" retrive token list first hit");
          updateTokenList(res);
  ...
  function resetBalances() {
    ...
    setShowBalance1(!1);
    setShowBalance2(!1);
    console.log("TOKEN_INIT.BAL()", TOKEN_INIT.BAL());
  ...
  function isNotOKToProceed() {
    ...
    let errMsg = !wallet.isConnected
      ? ERR.CONNECT_WALLET
      : rEqual(swap.token1_sym, MISC.SEL_TOKEN)
      ? ERR.SELECT_TOKEN_1
      : rEqual(swap.token2_sym, MISC.SEL_TOKEN)
      ? ERR.SELECT_TOKEN_2
      : "";
  ...
  async function approveWithMaxAmount(e) {
    ...
    if (localChainId == 338) {
      ...
      await TokenContract.approve(MISC.MAX_256, ADDRESS.ROUTER_CONTRACT2);
    } else if (localChainId == 43113) {
      await TokenContract.approve(MISC.MAX_256, ADDRESS.ROUTER_CONTRACT3);
    }
    setIsErr(!1);
    setIsFetching(!1);
    setTokenApproved(!0);
  ...
  async function tryNormalizePair(p) {
    ...
    if (localChainId == 338) {
      ...
      // else if (
      //   rEqual(p[0], ADDRESS.CST_TOKEN) ||
      //   rEqual(p[1], ADDRESS.CST_TOKEN)
      // ) {
      ...
      // check if both the non-cst tokens has pair with cst token
        ...
        pAddr = await FactoryContract.getPair(
          p[0],
          ADDRESS.CST_FAUCET_CONTRACT
        );
        let pAddr2 = await FactoryContract.getPair(
          p[1],
          ADDRESS.CST_FAUCET_CONTRACT
        );
        if (isAddr(pAddr) && isAddr(pAddr2))
          return [p[0], ADDRESS.CST_FAUCET_CONTRACT, p[1]];
      }
    } else if (localChainId == 97) {
      if (isAddr(pAddr)) return p;
      else {
        ...
        if (isAddr(pAddr) && isAddr(pAddr2))
          return [p[0], ADDRESS.CST_FAUCET_CONTRACT2, p[1]];
      }
    } else if (localChainId == 43113) {
      if (isAddr(pAddr)) return p;
      else {
        ...
        if (isAddr(pAddr) && isAddr(pAddr2))
          return [p[0], ADDRESS.CST_FAUCET_CONTRACT3, p[1]];
      }
    }
    return [];
  }
  ...
  async function setSwapPrerequisites(pr, fetchedAmount, xactIn) {
    ...
    let xactAmount = toStd(
      typedValueGlobal * 10 ** (xactIn ? sellTokenDec : buyTokenDec)
    );
    let thAmount = getThresholdAmountFromTolerance(
      fetchedAmount,
      swap.slippage,
      xactIn,
      xactIn ? buyTokenDec : sellTokenDec
    );
    log.s(btnText);
    setPair(pr);
  ...
  useEffect(
    (_) => {
      if (window.hasLoaded) {
        (async (_) => {
          setTokenIp(
            `${toFixed(swap.token2_value.actual, MISC.OTHER_TOKEN_DEC_PLACES)}`,
            TOKEN.A
          );
  ...
  console.log("swap.token1_value1111111", swap.token1_value);
  console.log("swap.token1_value2222222222", swap.token2_value);
  ...
  async function setOtherTokenValue(ipNum, isUpsideDown, _pair) {
    console.log("ipNum, isUpsideDown, _pair", ipNum, isUpsideDown, _pair);
    ...
    if (notEmpty(typedValueGlobal) && notZero(typedValueGlobal)) {
      ...
      try {
        const addrList = isUpsideDown
          ? [swap.token2_addr, swap.token1_addr] //CST_FAUCET_CONTRACT2
          : [swap.token1_addr, swap.token2_addr];
        log.i("addrListttttttttt:", addrList);
        const pair = await tryNormalizePair(addrList);
        console.log("pairRRRRRRR", pair);
        ...
        if (rEqual(ipNum, TOKEN.B)) {
          const isLiqLow = await isLiquidityLowForPair(
            pair,
            toBigNum(param[0])
          );
          if (isLiqLow) {
        ...
        let fetchedAmounts = await (xactIn
        ...
        console.log("stdRaiseB_pair", stdRaiseBy("1", dec), _pair);
        const xchangePrice = toFixed(
          toDec(
            (
              await RouterContract.getAmountsIn([stdRaiseBy("1", dec), _pair])
            )[0],
            dec
          ),
          MISC.XCHANGE_PRICE_DEC_PLACES
        );
        console.log("xchangePrice", xchangePrice);
      ...
      return !0;
    } else return handleInputErr(ERR.NO_INPUT, !0, "", otherTokenNum, !0);
  }
  ...
  async function getAndShowPlayerInfo(player) {
    ...
    if (rEqual(swap.tokenInfoForUI.addr, player.addr))
      return setShowPlayerInfo(!0);
  ...
  async function searchOrImportToken(token) {
    // console.log("searchOrImportToken", token);
    const localChainId = localStorage.getItem("staticNwId");
    ...
    const players = swap.players.filter((player) =>
      rEqual(player.addr, token.addr)
    );
    log.i("Playersrrrrrrrrr:", players);
  ...
  async function checkIfCSTClaimed(account = wallet.priAccount) {
    setIsCSTClaimed(await FaucetContract.hasClaimed(account));
  }

  async function claimCST(e) {
    e.preventDefault();
    console.log("claimCSTPPPPPPPPPPP");
    try {
      setIsClaiming(!0);
      ...
      console.log("eeeeeeeeeeeee", Err.handle(e));
      Err.handle(e);
      setIsClaiming(!1);
    }
  }
  ...
  async function fetchBalanceOf(selectedToken, addrList) {
    ...
    console.log("selectedToken, TOKEN.A", selectedToken, TOKEN.A);
    if (rEqual(selectedToken, TOKEN.A)) {
      TokenContract.init(addrList[0]);
      dec = await TokenContract.decimals();
      console.log("decMMMMMM", dec);
      console.log("preaccount", wallet.priAccount);
      bal = (await TokenContract.balanceOf(wallet.priAccount)).toString();
      console.log("preaccount_bal", bal);
      b.actual = toDec(bal, dec);
      b.ui = toFixed(b.actual, 2);
      console.log("setToken1_bal", b);
      setToken1_bal(b);
  ...
  async function setToMaxAmount(selectedToken) {
    if (isNotOKToProceed()) return;
    if (rEqual(selectedToken, TOKEN.A)) {
      console.log("Token.AAAAA", TOKEN.A);
  ...
  function eventListeners() {
    ...
    const onTokenSelected = (e) => {
      ...
      console.log("e.detail.addrList", e.detail.addrList);
    ...
    const onChainChanged = async (e) => {
      console.log("chain changed event", e.detail.isInvalid);
      setIsInvalidNetwork(!1);
    };
