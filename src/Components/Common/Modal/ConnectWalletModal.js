...
import {
  PROVIDER_EVENT,
  WALLET_TYPE,
} from "../../../services/constants/wallet";
...
const truncAddr = (a) =>
  isDefined(a) && notEmpty(a)
    ? `${a.slice(0, 5)}..${a.slice(39, 42)}`
    : MISC.CONNECT_TTL;
...
const ConnectWalletModal = (props) => {
  ...
  const connect2wallet = async (walletType) => {
    try {
      ...
      console.log("ErrErrErrErrErrErr", e);
      Err.handle(e);
    }
  };
  ...
  return (
    ...
    >
      <ul>
        <li>
          <button
            className={`img--wallet-logo img--wallet-logo-mm${
              wallet.isConnected &&
              rEqual(WALLET_TYPE.METAMASK, wallet.walletType)
                ? " disconnect-wallet"
                : wallet.isConnected &&
                  rEqual(WALLET_TYPE.WALLET_CONNECT, wallet.walletType)
                ? " disabled--wallet-btn"
                : ""
            }`}
            disabled={
        ...
        <li>
          <button
            className={`img--wallet-logo img--wallet-logo-wc${
              wallet.isConnected &&
              rEqual(WALLET_TYPE.WALLET_CONNECT, wallet.walletType)
                ? " disconnect-wallet"
                : wallet.isConnected &&
