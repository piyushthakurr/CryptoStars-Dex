...
const isNaNy = (n) =>
  rEqual(n, NaN) || rEqual(n, "NaN") || rEqual(`${n}`, "NaN");
...
const evDispatch = (eName, d) =>
  document.dispatchEvent(new CustomEvent(eName, { detail: d }));
...
const raiseBy = (v, dec) =>
  Number(v) * 10 ** (isDefined(dec) && notEmpty(dec) ? dec : 18);

const stdRaiseBy = (v, dec) => toStd(raiseBy(v, dec));

const toBigNum = (v) => (v instanceof BigNumber ? v : BigNumber.from(v));

const tStampJs = (offset) =>
  Date.now() + (isDefined(offset) ? offset : 0) * 1000;

const tStampNix = (offset) =>
  Math.ceil(Date.now() / 1000) + (isDefined(offset) ? offset : 0);
