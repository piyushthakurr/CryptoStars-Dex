...
let Wallet = {
  ...
  init: async function (walletType) {
    ...
    if (notDefined(this._provider) || notEqual(this.walletType, walletType)) {
      if (rEqual(walletType, WALLET_TYPE.METAMASK)) {
        ...
        this._accounts = (
          await this.provider.send(WALLET_METH.REQ_ACCOUNTS, [])
        ).map((a) => getAddress(a));
        ...
        this._provider = this._provider = new ethers.providers.Web3Provider(
          wcProvider,
          "any"
        );
        this._accounts = (
          await this.provider.send(WALLET_METH.REQ_ACCOUNTS_INFURA, [])
        ).map((a) => getAddress(a));
  ...
  ensureChain: async function () {
    ...
    if (localChainId == 338) {
      ...
      log.i("switching to cronos chain338");
    } else if (localChainId == 97) {
      evDispatch(EVENT.CHAIN_CHANGE, { isInvalid: !0 });
      await this.provider.send(WALLET_METH.ADD_CHAIN, [
        WALLET_PARAM.ADD_CHAIN_TBNB,
      ]);
      log.i("switching to bnb chain97");
    } else if (localChainId == 43113) {
      evDispatch(EVENT.CHAIN_CHANGE, { isInvalid: !0 });
      await this.provider.send(WALLET_METH.ADD_CHAIN, [
        WALLET_PARAM.ADD_CHAIN_AVAX,
      ]);
      log.i("switching to ava chain43113");
    } else {
      evDispatch(EVENT.CHAIN_CHANGE, { isInvalid: !1 });
    }
